...
| **API** | Render: **Root Directory empty**. Build `pip install -r requirements.txt`. Start **`bash start.sh`**. If the dashboard still says `uvicorn app.main:app`, add env `PYTHONPATH=backend` (the root `app/` shim also handles that command). Health `/health`. Python 3.11.11. |
